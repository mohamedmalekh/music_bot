...
if not TOKEN or not GROUP_ID_STR:
    logger.error("FATAL: TELEGRAM_BOT_TOKEN or TELEGRAM_GROUP_ID not set")
    sys.exit(1)
try:
...
def list_new_youtube_videos(channel_url):
    ...
    if "/channel/" in channel_url:
        ...
        logger.warning("Unknown YouTube URL format, using last segment as channel ID")
...
def fetch_youtube_mp3(url):
    # Liste des instances Invidious fiables
    INVIDIOUS_INSTANCES = [
    ...
    # Choisir une instance au hasard
    base_url = random.choice(INVIDIOUS_INSTANCES)

    # Extraire l'ID de la vidéo YouTube
    video_id = url.split("v=")[-1].split("&")[0]

    # Construire l'URL API pour obtenir les streams
    api_url = f"{base_url}/api/v1/streams/{video_id}"

    print(f"[INFO] Using Invidious instance: {base_url}")
    ...
    try:
        ...
        # Chercher un stream audio (format audio/webm ou audio/mp4)
        audio_url = None
        ...
        print(f"[INFO] Downloading audio stream: {audio_url}")
        ...
        out, _ = process.communicate(audio_data.content)
        ...
        print(f"[ERROR] Failed to fetch MP3: {e}")
        raise
